...
script_name <- "admixture-plot-nature_1.R"
...
usage <- function() {
  cat("Usage: Rscript admixture-plot-nature_1.R --q-dir DIR --fam FILE --k-min INT --k-max INT --cv-out FILE --structure-out FILE --cv-plot-out FILE [--pop-info FILE] [--show-sample-names]\n\n")
  cat("Options:\n")
...
if (!is.null(opts[["pop-info"]]) && nzchar(opts[["pop-info"]])) {
  ...
  group <- pop$population[match(samples, pop$sample)]
  group[is.na(group)] <- "Ungrouped"
  order_idx <- order(group, samples)
}
