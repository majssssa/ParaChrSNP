...
修改说明：
1. 原始脚本路径：scripts/parachrsnp-report_3.py
2. 本版本在统一 HTML 报告中新增 SNP density 染色体热图和统计表展示。
3. 保留上一版本的 CNV、Pi、ADMIXTURE、LD decay 图形展示逻辑。
4. PNG/SVG 图片会直接嵌入报告，PDF 图会通过网页只读输出路由进行预览。
...
AUTHOR = "Junpeng Ma 1527552938@qq.com"
SCRIPT_NAME = "parachrsnp-report_4.py"
SCRIPT_FUNCTION = "Generate ParaChrSNP HTML report with embedded workflow and SNP density figures."
...
def read_cnv_summary(path):
    # 读取 CNV summary 表，统计总 CNV 数、每类 CNV 数和总长度。
    summary = {
        "total_cnv": 0,
        "deletion": 0,
        "duplication": 0,
    ...
    with open(path, "r", encoding="utf-8", errors="replace") as handle:
        ...
        for row in reader:
            ...
            name = row.get("name", "")
            try:
                count = int(float(row.get("cnv_count", 0)))
            except ValueError:
                count = 0
            if section == "sample":
                samples.add(name)
                summary["total_cnv"] += count
                try:
                    summary["total_length_bp"] += int(float(row.get("total_length_bp", 0)))
                except ValueError:
                    pass
            elif section == "type":
                summary[name] = count
            elif section == "chromosome":
                chromosomes.add(name)
    summary["sample_count"] = len(samples)
    summary["chromosome_count"] = len(chromosomes)
    return summary
...
def output_url(path):
    # 报告位于 reports/ 目录中，使用相对路径保证直接打开 HTML 文件时也能访问结果。
    return "../" + path.replace("\\", "/").lstrip("/")


def preview_path(path):
    # PDF 结果优先使用同名 SVG/PNG 作为报告内预览，避免直接打开 HTML 时 PDF iframe 路径失效。
    if not path:
        return ""
    lower = path.lower()
    if lower.endswith((".png", ".svg")) and os.path.exists(path):
        return path
    if lower.endswith(".pdf"):
        stem = path[:-4]
        for candidate in [stem + ".png", stem + ".svg"]:
...
def figure_card(title, path, description=""):
    ...
    if preview and preview != path:
        extra_links = f' | <a href="{html.escape(output_url(preview))}" target="_blank" rel="noopener">preview</a>'
    return (
        '<div class="figure-card">'
        f"<h3>{title_html}</h3>"
        f"{desc_html}"
        f"{media}"
        f'<p class="file-link"><a href="{html.escape(output_url(path))}" target="_blank" rel="noopener">{path_html}</a>{extra_links}</p>'
        "</div>"
    )
...
def collect_figure_cards(params, samples):
    ...
    if params.get("cnv", {}).get("enabled", False):
        cards.extend([
            figure_card(
                "CNV count by sample",
                "cnv/figures/cnv_count_by_sample.pdf",
                "Number of CNVnator calls in each sample.",
            ),
            figure_card(
                "CNV length distribution",
                "cnv/figures/cnv_length_distribution.pdf",
                "Distribution of CNV lengths on a log10 scale.",
            ),
            figure_card(
                "CNV count by chromosome",
                "cnv/figures/cnv_count_by_chromosome.pdf",
                "Chromosomal distribution of CNVnator calls.",
            ),
        ])

    if params.get("pi", {}).get("enabled", False):
        pi_dir = params.get("pi", {}).get("output_dir", "pi")
        cards.append(
            figure_card(
                "Windowed nucleotide diversity",
                pi_dir + "/figures/pi_manhattan.pdf",
                "Genome-wide Manhattan-style plot of windowed nucleotide diversity (Pi).",
            )
        )

    if params.get("snp_density", {}).get("enabled", False):
        density_dir = params.get("snp_density", {}).get("output_dir", "snp_density")
        cards.append(
            figure_card(
                "SNP density",
                density_dir + "/figures/snp_density.pdf",
                "Number of filtered SNP sites in each fixed-width chromosome window.",
            )
        )

    if params.get("admixture", {}).get("enabled", False):
        admixture_dir = params.get("admixture", {}).get("output_dir", "admixture")
        cards.extend([
            figure_card(
                "ADMIXTURE population structure",
                admixture_dir + "/figures/admixture_structure.pdf",
                "Stacked ancestry components inferred by ADMIXTURE across configured K values.",
            ),
            figure_card(
                "ADMIXTURE cross-validation error",
                admixture_dir + "/figures/admixture_cv_error.pdf",
                "Cross-validation error used to compare ADMIXTURE K values.",
            ),
        ])

    if params.get("ld_decay", {}).get("enabled", False):
        ld_decay_dir = params.get("ld_decay", {}).get("output_dir", "ld_decay")
        cards.append(
            figure_card(
                "LD decay",
                ld_decay_dir + "/figures/ld_decay.pdf",
...
def main():
    ...
    cnv_summary = read_cnv_summary("cnv/combined.cnv.summary.tsv")
    if params.get("cnv", {}).get("enabled", False):
        rows_for_tsv.extend([
            ("cnv", "total_cnv", cnv_summary["total_cnv"]),
            ("cnv", "deletion", cnv_summary.get("deletion", 0)),
            ("cnv", "duplication", cnv_summary.get("duplication", 0)),
            ("cnv", "total_length_bp", cnv_summary["total_length_bp"]),
        ])
    ...
    if params.get("cnv", {}).get("enabled", False):
        output_rows.extend([
            ["CNV combined table", "cnv/combined.cnv.tsv", file_size("cnv/combined.cnv.tsv")],
            ["CNV summary table", "cnv/combined.cnv.summary.tsv", file_size("cnv/combined.cnv.summary.tsv")],
            ["CNV count by sample", "cnv/figures/cnv_count_by_sample.pdf", file_size("cnv/figures/cnv_count_by_sample.pdf")],
            ["CNV length distribution", "cnv/figures/cnv_length_distribution.pdf", file_size("cnv/figures/cnv_length_distribution.pdf")],
            ["CNV count by chromosome", "cnv/figures/cnv_count_by_chromosome.pdf", file_size("cnv/figures/cnv_count_by_chromosome.pdf")],
        ])
        for sample in samples:
            path = f"cnv/{sample}.cnv.vcf"
            output_rows.append([f"CNV VCF ({sample})", path, file_size(path)])

    if params.get("pi", {}).get("enabled", False):
        pi_dir = params.get("pi", {}).get("output_dir", "pi")
        output_rows.extend([
            ["Windowed Pi table", pi_dir + "/combined.windowed.pi.tsv", file_size(pi_dir + "/combined.windowed.pi.tsv")],
            ["Pi summary table", pi_dir + "/pi.summary.tsv", file_size(pi_dir + "/pi.summary.tsv")],
            ["Pi Manhattan plot", pi_dir + "/figures/pi_manhattan.pdf", file_size(pi_dir + "/figures/pi_manhattan.pdf")],
        ])
    if params.get("snp_density", {}).get("enabled", False):
        density_dir = params.get("snp_density", {}).get("output_dir", "snp_density")
        output_rows.extend([
            ["SNP density table", density_dir + "/snp_density.tsv", file_size(density_dir + "/snp_density.tsv")],
            ["SNP density plot", density_dir + "/figures/snp_density.pdf", file_size(density_dir + "/figures/snp_density.pdf")],
        ])
    if params.get("admixture", {}).get("enabled", False):
        admixture_dir = params.get("admixture", {}).get("output_dir", "admixture")
        output_rows.extend([
            ["ADMIXTURE CV errors", admixture_dir + "/cv_errors.tsv", file_size(admixture_dir + "/cv_errors.tsv")],
            ["ADMIXTURE structure plot", admixture_dir + "/figures/admixture_structure.pdf", file_size(admixture_dir + "/figures/admixture_structure.pdf")],
            ["ADMIXTURE CV plot", admixture_dir + "/figures/admixture_cv_error.pdf", file_size(admixture_dir + "/figures/admixture_cv_error.pdf")],
        ])
    if params.get("ld_decay", {}).get("enabled", False):
        ld_decay_dir = params.get("ld_decay", {}).get("output_dir", "ld_decay")
        output_rows.extend([
            ["LD decay table", ld_decay_dir + "/combined.ld_decay.tsv", file_size(ld_decay_dir + "/combined.ld_decay.tsv")],
            ["LD decay plot", ld_decay_dir + "/figures/ld_decay.pdf", file_size(ld_decay_dir + "/figures/ld_decay.pdf")],
        ])
...
<h2>CNV Summary</h2>
{html_table(["Metric", "Value"], [
    ["CNV module enabled", params.get("cnv", {}).get("enabled", False)],
    ["Total CNV calls", fmt_int(cnv_summary["total_cnv"])],
    ["Deletions", fmt_int(cnv_summary.get("deletion", 0))],
    ["Duplications", fmt_int(cnv_summary.get("duplication", 0))],
    ["Total CNV length (bp)", fmt_int(cnv_summary["total_length_bp"])],
])}
